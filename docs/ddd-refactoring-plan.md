...
> **繋ぎ込みの方針（T0-5 の決定を受けて）**
> presentation 層から domain 層への直接 import は禁止したため、Phase 1 で作った
> ドメインモデルを App から直接は使えない。Phase 3 までまとめて繋ぎ込むと一括変更が
> 大きくなり「毎ステップ動く」方針と噛み合わないため、**各タスクで application 層に
> 薄いユースケースを置き、その場で UI に繋ぐ**。Phase 3 はそれらを整理・命名する回になる。
...
- [x] **T3-3** 組み立てを `main.tsx`（composition root）に集約し、Context でユースケースを注入する

## Phase 4: プレゼンテーション層を薄くする

- [x] **T4-1** `App` からロジックを剥がし、`useTasks` フックがユースケースを呼ぶだけにする
- [x] **T4-2** `Form` / `TaskItem` を DTO 前提に整理する
      あわせて T0-2 で見つけたアクセシブル名の不具合と、変更フォームの初期値を直した
- [x] **T4-3** `TaskName` の不変条件違反をエラーメッセージとして UI に表示する

## Phase 5: 発展（DDD の議論を味わう）

- [x] **T5-1** 集約境界の検討: 集約ルートは `Task` か `TaskList` か
      `TaskList` を集約ルートと定め、不変条件（重複禁止）と変更の入口を実装した
- [x] **T5-2** ドメインイベント（`TaskCompleted`）の導入
      購読者として完了履歴（`CompletionHistory`）を作り、画面に「最近完了したタスク」を出した
- [x] **T5-3** ドメインサービス／仕様パターンが必要になるケースの検討
      どちらも現時点では不要と判断。根拠と「必要になる条件」を docs/pattern-decisions.md に記録
- [x] **T5-4** 境界づけられたコンテキストの考察
      実質1つと結論。完了履歴を題材にした分析を docs/bounded-contexts.md に記録
